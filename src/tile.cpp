...
template <typename T>
void elementwise_max(
    T *mp_full, uint64_t merge_start, uint64_t tile_sz, T *to_merge,
    uint64_t index_offset,
    std::function<bool(float, float, int, int)> updateChecker) {
  for (int i = 0; i < tile_sz; ++i) {
    mp_entry e1, e2;
    e1.ulong = mp_full[i + merge_start];
    e2.ulong = to_merge[i];
    if (updateChecker(e1.floats[0], e2.floats[0], i + merge_start,
...
void Tile::MergeTileIntoFullProfile(Profile *tile_profile, uint64_t position,
  ...
  switch (_info->profile_type) {
    case PROFILE_TYPE_SUM_THRESH:
      ...
      return;
    case PROFILE_TYPE_1NN_INDEX: {
      std::function<bool(float, float, int, int)> reductionFunct;

      if (reduction == Reduction::LEFT)
        reductionFunct = comparison_left;
      else if (reduction == Reduction::RIGHT)
        reductionFunct = comparison_right;
      else
        reductionFunct = comparison_full;

      elementwise_max<uint64_t>(
          full_profile->data.uint64_value.data(), position, length,
          tile_profile->data.uint64_value.data(), index_start, reductionFunct);
    }
      return;
    case PROFILE_TYPE_1NN:
...
void Tile::MergeProfile(Profile *profile_a, std::mutex &a_lock,
  ...
  if (!_info->self_join && _info->reduction_type == Reduction::LEFT_RIGHT) {
    throw SCAMPException(
        "Left-Right reduction is only allowed when it is self_join");
  }
  if (_info->reduction_type == Reduction::LEFT_RIGHT) {
    ...
    MergeTileIntoFullProfile(&_profile_b_tile, _current_tile_row,
                             _current_tile_height - _info->mp_window + 1,
                             profile_b, _current_tile_col, a_lock,
                             Reduction::RIGHT);
  } else {
    // Merge result
    MergeTileIntoFullProfile(&_profile_a_tile, _current_tile_col,
