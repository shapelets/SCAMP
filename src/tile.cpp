...
void elementwise_max(T *mp_full, uint64_t merge_start, uint64_t tile_sz,
  ...
  for (int i = 0; i < tile_sz; ++i) {
    ...
    e2.ulong = to_merge[i];
    auto presentValue = e1.floats[0];
    auto updateValue = e2.floats[0];
    auto presentIndex = i + merge_start;
    auto updateIndex = e2.ints[1] + index_offset;
    bool update = false;
    switch (reduction_type) {
      case Reduction::FULL:
        update = presentValue < updateValue;
        break;
      case Reduction::RIGHT:
        update = presentValue < updateValue && presentIndex < updateIndex;
        break;
      case Reduction::LEFT:
        update = presentValue < updateValue && presentIndex > updateIndex;
        break;
  ...
  switch (_info->profile_type) {
    ...
    case PROFILE_TYPE_1NN_INDEX: {
      elementwise_max<uint64_t>(
          ...
          tile_profile->data.uint64_value.data(), index_start, reduction);
    }
      return;
    case PROFILE_TYPE_1NN:
