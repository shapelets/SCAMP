...
class SCAMP_Operation {
 ...
 public:
  SCAMP_Operation(size_t Asize, size_t Bsize, size_t window_sz,
                  ...
                  SCAMPProfileType profile_type, Reduction reduction,
      ...
      : _info(Asize, Bsize, window_sz, max_tile_size, selfjoin, t, start_row,
              start_col, args_, profile_type, reduction, keep_rows,
              compute_rows, compute_cols, is_aligned, silent_mode,
              dev.size() + num_threads),
        _completed_tiles(0),
        _profile_a(pA),
        _profile_b(pB),
        _devices(dev),
